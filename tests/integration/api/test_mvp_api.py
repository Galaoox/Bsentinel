...
def test_create_duplicate_book_returns_409(client):
    ...
    duplicate = client.post("/api/v1/books", json=payload)
...
def test_delete_and_restore_book(client):
    create = client.post("/api/v1/books", json={"url": "https://www.buscalibre.com.co/libro-clean-code"})
...
def test_history_and_price_comparison_endpoints(client):
    ...
    history = client.get(f"/api/v1/books/{book_id}/history")
    assert history.status_code == 200
    assert history.json()["meta"]["total"] >= 1

    comparison = client.get(f"/api/v1/books/{book_id}/price-comparison")
...
def test_archive_job_flow(client):
    ...
    create_job = client.post(
        "/api/v1/retention/archive-jobs",
        json={"target": "price_history", "older_than_days": 1, "min_active_records_per_book": 1},
    )
    assert create_job.status_code == 202
    job_id = create_job.json()["id"]

    job_status = client.get(f"/api/v1/retention/archive-jobs/{job_id}")
