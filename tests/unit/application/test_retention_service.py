...
@pytest.mark.asyncio
async def test_retention_service_rejects_invalid_parameters():
    storage = InMemoryStore()
    service = RetentionService(jobs=InMemoryArchiveJobRepository(storage))

    with pytest.raises(ValidationError):
        await service.create_archive_job(older_than_days=0, min_active_records_per_book=1)

    with pytest.raises(ValidationError):
        await service.create_archive_job(older_than_days=1, min_active_records_per_book=0)
