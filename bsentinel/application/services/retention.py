...
class RetentionService:
    ...
    async def create_archive_job(self, older_than_days: int, min_active_records_per_book: int) -> dict:
        ...
        job = await self.jobs.run(job.id, older_than_days, min_active_records_per_book)
        return asdict(job)

    async def get_archive_job(self, job_id: UUID) -> dict:
